...
colnames(df.expression_matrix.clean) <- with(df.columns, paste(donor_id, structure_acronym, stage, age, sep="_")) # NEW - added 
...
############################# ** NEW 12/01/2014 
df.expression_matrix.clean.melt$age <- as.factor(sapply(variable_split, "[[", 4)) #### ** NEW 12/01/2014 
...
## sorting stage levels **OBS**: HIGH RES!
df.expression_matrix.clean.melt$stage <- with(df.expression_matrix.clean.melt, factor(stage, levels(stage)[match(order.stages, levels(stage))]))
############################# ** NEW 12/01/2014  - dummy variable - correct this later **###########################
...
#save(df.expression_matrix.clean.unprocessed, df.expression_matrix.clean.melt.unprocessed, file="RData/data_rnaseq_expression_unprocessed_high_res.RData")
...
save(df.expression_matrix.clean, df.expression_matrix.clean.melt, file="RData/data_rnaseq_expression_processed_high_res.RData") # df.expression_matrix.clean, df.expression_matrix.clean.melt
